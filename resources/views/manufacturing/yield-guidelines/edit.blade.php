...
@section('title', 'Edit Yield Guideline')
    ...
    <div class="container-xl">
        <div class="row g-2 align-items-center">
            <div class="col">
                ...
                <h2 class="page-title">
                    Edit Yield Guideline
                </h2>
            </div>
            <div class="col-auto ms-auto d-print-none">
                <div class="btn-list">
                    <a href="{{ route('manufacturing.yield-guidelines.show', $yieldGuideline) }}" class="btn btn-outline-secondary">
                        <i class="far fa-arrow-left"></i>&nbsp;
                        <span class="d-none d-sm-inline">Back to Guideline</span>
        ...
        <div class="row row-deck row-cards">
            <div class="col-12">
                <div class="card">
                    ...
                    <form method="POST" action="{{ route('manufacturing.yield-guidelines.update', $yieldGuideline) }}">
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select name="from_stage" class="form-select @error('from_stage') is-invalid @enderror" required onchange="updateFromItems()">
                                            ...
                                            <option value="adonan" {{ old('from_stage', $yieldGuideline->from_stage) === 'adonan' ? 'selected' : '' }}>Adonan</option>
                                            <option value="gelondongan" {{ old('from_stage', $yieldGuideline->from_stage) === 'gelondongan' ? 'selected' : '' }}>Gelondongan</option>
                                            <option value="kerupuk_kg" {{ old('from_stage', $yieldGuideline->from_stage) === 'kerupuk_kg' ? 'selected' : '' }}>Kerupuk Kg</option>
                                        </select>
                                        @error('from_stage')
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select name="to_stage" class="form-select @error('to_stage') is-invalid @enderror" required onchange="updateToItems()">
                                            ...
                                            <option value="gelondongan" {{ old('to_stage', $yieldGuideline->to_stage) === 'gelondongan' ? 'selected' : '' }}>Gelondongan</option>
                                            <option value="kerupuk_kg" {{ old('to_stage', $yieldGuideline->to_stage) === 'kerupuk_kg' ? 'selected' : '' }}>Kerupuk Kg</option>
                                            <option value="packing" {{ old('to_stage', $yieldGuideline->to_stage) === 'packing' ? 'selected' : '' }}>Packing</option>
                                        </select>
                                        @error('to_stage')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                ...
                                <div class="col-md-6 col-lg-4">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control @error('yield_quantity') is-invalid @enderror" 
                                               name="yield_quantity" value="{{ old('yield_quantity', $yieldGuideline->yield_quantity) }}" step="0.001" min="0.001" required>
                        ...
                        <div class="card-footer text-end">
                            <div class="d-flex">
                                <a href="{{ route('manufacturing.yield-guidelines.show', $yieldGuideline) }}" class="btn btn-link">Cancel</a>
                                <button type="submit" class="btn btn-primary ms-auto">Update Yield Guideline</button>
                            </div>
                        </div>
                    </form>
                </div>
...
function initializeFromItemSelect() {
    ...
    const currentValue = fromItemSelect.value || fromItemSelect.getAttribute('data-old-value');
    ...
    filteredFromOptions.forEach(opt => {
        ...
        // Preserve current value if it exists and is still valid
...
function initializeToItemSelect() {
    ...
    // Clear and rebuild select options
    const currentValue = toItemSelect.value || toItemSelect.getAttribute('data-old-value');
...
function updateFromItems() {
    ...
    if (fromItemTomSelect) {
        ...
        currentValue = fromItemSelect.value || fromItemSelect.getAttribute('data-old-value');
    }
...
function updateToItems() {
    ...
    if (toItemTomSelect) {
        ...
        // Store current value from the select element itself
        currentValue = toItemSelect.value || toItemSelect.getAttribute('data-old-value');
    }
