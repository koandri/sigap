                            ...
                            <div class="table-responsive">
                                <table class="table table-vcenter" id="step1-table">
                                    ...
                                    <tbody id="step1-tbody">
                                        ...
                                        <tr class="step1-row">
                                            <td>
                                                <select name="step1[{{ $index }}][dough_item_id]" class="form-select dough-item-select" required onchange="loadRecipes(this, {{ $index }})">
                                            ...
                                            <td>
                                                <select name="step1[{{ $index }}][recipe_id]" class="form-select recipe-select" id="recipe-select-{{ $index }}" onchange="handleRecipeSelection(this, {{ $index }})">
                                                    <option value="">Select Recipe</option>
                                                </select>
                                                <div class="mt-2">
                                                    <label class="form-check form-check-inline">
                                                        <input type="checkbox" class="form-check-input" name="step1[{{ $index }}][is_custom_recipe]" 
                                                               {{ $step1->is_custom_recipe ? 'checked' : '' }} onchange="toggleCustomRecipe(this, {{ $index }})">
                                                        <span class="form-check-label">Custom Recipe</span>
                                                    </label>
                                                </div>
                                                <div class="custom-recipe-fields mt-2" id="custom-recipe-{{ $index }}" style="display: {{ $step1->is_custom_recipe ? 'block' : 'none' }};">
                                                    <input type="text" name="step1[{{ $index }}][recipe_name]" class="form-control form-control-sm mb-1" 
                                                           value="{{ $step1->recipe_name }}" placeholder="Recipe Name">
                                                    <input type="date" name="step1[{{ $index }}][recipe_date]" class="form-control form-control-sm" 
                                                           value="{{ $step1->recipe_date->format('Y-m-d') }}">
                                                </div>
                                            </td>
                                            <td>
                                                <input type="number" name="step1[{{ $index }}][qty_gl1]" class="form-control" step="0.001" min="0" value="{{ $step1->qty_gl1 }}" required>
                                            </td>
                                            <td>
                                                <input type="number" name="step1[{{ $index }}][qty_gl2]" class="form-control" step="0.001" min="0" value="{{ $step1->qty_gl2 }}" required>
                                            </td>
                                            <td>
                                                <input type="number" name="step1[{{ $index }}][qty_ta]" class="form-control" step="0.001" min="0" value="{{ $step1->qty_ta }}" required>
                                            </td>
                                            <td>
                                                <input type="number" name="step1[{{ $index }}][qty_bl]" class="form-control" step="0.001" min="0" value="{{ $step1->qty_bl }}" required>
                                            </td>
                                            <td>
...
function addStep1Row() {
    ...
    const oldOnchange = doughSelect.getAttribute('onchange');
    doughSelect.setAttribute('onchange', oldOnchange.replace(/\d+/, rowIndex));
    
    const recipeSelect = row.querySelector('.recipe-select');
    const oldRecipeOnchange = recipeSelect.getAttribute('onchange');
    recipeSelect.setAttribute('onchange', oldRecipeOnchange.replace(/\d+/, rowIndex));
    
    const customRecipeCheck = row.querySelector('[name*="[is_custom_recipe]"]');
    const oldCustomOnchange = customRecipeCheck.getAttribute('onchange');
    customRecipeCheck.setAttribute('onchange', oldCustomOnchange.replace(/\d+/, rowIndex));
...
function loadRecipes(select, index) {
    ...
    fetch(`{{ route('manufacturing.production-plans.recipes') }}?dough_item_id=${doughItemId}`)
        ...
        .then(data => {
            ...
            // Select existing recipe if any
            const existingRecipeId = {{ $productionPlan->step1->first()->recipe_id ?? 'null' }};
            if (existingRecipeId) {
                recipeSelect.value = existingRecipeId;
            }
        })
        .catch(error => {
...
// Load recipes for existing rows on page load
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.dough-item-select').forEach((select, index) => {
        if (select.value) {
            loadRecipes(select, index);
        }
    });
});
</script>
