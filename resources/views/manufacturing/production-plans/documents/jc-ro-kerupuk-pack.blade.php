...
<head>
    ...
    <title>JC/RO Kerupuk Pack - {{ $plan->plan_date->format('d M Y') }}</title>
    ...
    <div class="page-header d-print-none">
        <div class="container-xl">
            <div class="row g-2 align-items-center">
                <div class="col">
                    <h2 class="page-title">JC/RO Kerupuk Pack</h2>
                </div>
    ...
    <div class="page-body">
        <div class="container-xl">
            ...
            <div class="mt-4">
                ...
                <h4 class="mb-3">Kerupuk Kg yang Digunakan untuk Memproduksi Kerupuk Pack</h4>
                    <table>
                        <thead>
                            <tr>
                                <th>No</th>
                        ...
                        <tbody>
                            @forelse($jcData['kerupukKg'] ?? [] as $index => $item)
                            <tr>
                                <td class="text-center">{{ $index + 1 }}</td>
                                <td class="barcode-cell">
                                    @if($item['item']->accurate_id)
                                        <svg class="barcode-{{ $index }}" data-barcode="{{ $item['item']->accurate_id }}"></svg>
                                    @else
                    ...
                    <h3 class="mt-4 mb-3">Bahan Kemasan yang Digunakan untuk Memproduksi Kerupuk Pack</h3>
                    <table>
                        <thead>
                            <tr>
                                <th>No</th>
                        ...
                        <tbody>
                            @forelse($jcData['packingMaterials'] ?? [] as $index => $material)
                            <tr>
                                <td class="text-center">{{ $index + 1 }}</td>
                                <td class="barcode-cell">
                                    @if($material['item']->accurate_id)
                                        <svg class="barcode-mat-{{ $index }}" data-barcode="{{ $material['item']->accurate_id }}"></svg>
                                    @else
                                        <span class="text-muted">-</span>
                                    @endif
                                </td>
                                <td>{{ $material['item']->name }}</td>
                                <td class="text-right">{{ number_format($material['quantity'], 0) }}</td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="4" class="text-center">Tidak ada data</td>
            ...
            <div class="mt-5">
                ...
                <h4 class="mb-3">Kerupuk Pack yang Diproduksi</h4>
                    <table>
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Kode</th>
                                <th>Nama Kerupuk Pack</th>
                                <th class="text-right">Jumlah</th>
                                <th class="text-right">Persentase (%)</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($roData['kerupukPack'] ?? [] as $index => $item)
                            <tr>
                                <td class="text-center">{{ $index + 1 }}</td>
                                <td class="barcode-cell">
                                    @if($item['item']->accurate_id)
