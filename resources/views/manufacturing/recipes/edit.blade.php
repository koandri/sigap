            ...
            <div class="col-auto ms-auto d-print-none">
                <div class="btn-list">
                    <a href="{{ route('manufacturing.recipes.index') }}" class="btn btn-outline-secondary">
                        ...
                        <span class="d-none d-sm-inline">Back to Recipes</span>
...
<div class="page-body">
    <div class="container-xl">
        ...
        <form method="POST" action="{{ route('manufacturing.recipes.update', $recipe) }}" id="recipe-form">
            @csrf
            @method('PUT')
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('dough_item_id') is-invalid @enderror" 
                                            ...
                                            <option value="{{ $id }}" {{ old('dough_item_id', $recipe->dough_item_id) == $id ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control @error('name') is-invalid @enderror" 
                                               name="name" value="{{ old('name', $recipe->name) }}" placeholder="Enter recipe name" maxlength="100" required>
                                        @error('name')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="date" class="form-control @error('recipe_date') is-invalid @enderror" 
                                               name="recipe_date" value="{{ old('recipe_date', $recipe->recipe_date->format('Y-m-d')) }}" required>
                                        @error('recipe_date')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-vcenter" id="ingredients-table">
                                    ...
                                    <tbody id="ingredients-tbody">
                                        @php
                                            $oldIngredients = old('ingredients', $recipe->ingredients);
                                            $ingredientCount = $oldIngredients->count() > 0 ? $oldIngredients->count() : 1;
                                        @endphp
                                        @forelse($oldIngredients as $index => $ingredient)
                                        <tr class="ingredient-row">
                                            <td>
                                                <select name="ingredients[{{ $index }}][ingredient_item_id]" class="form-select ingredient-item-select" id="ingredient-item-select-{{ $index }}" required>
                                                    ...
                                                    <option value="{{ $id }}" 
                                                            {{ old("ingredients.{$index}.ingredient_item_id", $ingredient->ingredient_item_id ?? null) == $id ? 'selected' : '' }}>
                                                        {{ $label }}
                                                    </option>
                                                    @empty
                            ...
                            <small class="form-hint">Add the ingredients needed for this recipe. At least one ingredient is required.</small>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
let ingredientIndex = {{ $ingredientCount ?? (old('ingredients', $recipe->ingredients)->count() > 0 ? old('ingredients', $recipe->ingredients)->count() : 1) }};
...
function initTomSelects() {
    ...
    // Initialize all existing ingredient selects
    ...
    ingredientRows.forEach((row, index) => {
        ...
        if (selectElement && selectElement.id) {
            ...
            if (match) {
                initializeIngredientSelect(parseInt(match[1]));
            }
        }
    });
}
