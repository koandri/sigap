...
let allFormOptions = []; // Store all original form options

document.addEventListener('DOMContentLoaded', function() {
    // Store all form options before initializing Tom Select
    ...
    Array.from(selectElement.options).forEach(option => {
        if (option.value) { // Skip the placeholder
            allFormOptions.push({
                value: option.value,
                text: option.text,
                versionId: option.dataset.versionId,
...
function addFormToRequest() {
    ...
    if (existingIndex !== -1) {
        ...
        // Remove the option from dropdown to prevent duplication
        formSelectInstance.removeOption(documentId);
    }
...
function removeForm(index) {
    const removedForm = selectedForms[index];
    
    // Find the original form option data
    const originalOption = allFormOptions.find(opt => opt.value === removedForm.documentId);
    
    // Add the option back to the dropdown
    if (originalOption) {
        formSelectInstance.addOption({
            value: originalOption.value,
            text: originalOption.text,
            number: originalOption.number,
            department: originalOption.department,
            versionId: originalOption.versionId
        });
    }
